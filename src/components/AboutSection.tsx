...
export default function AboutSection() {
  return (
    <section id="about" className="relative min-h-screen py-24 overflow-hidden" aria-label="About the builder">
      <div className="relative max-w-7xl mx-auto px-6 py-10">
        ...
        <div className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
          <div className="grid gap-6 md:grid-cols-2" role="region" aria-label="Builder statistics">
            {stats.map((stat) => (
              <motion.div
                key={stat.label}
              ...
              >
                <div className="rounded-3xl border border-white/10 bg-white/5 p-8 shadow-neon-pink/10 backdrop-blur-xl">
                  ...
                  <p className="mt-4 text-4xl font-bold text-white" aria-label={`${stat.label}: ${stat.value}`}>{stat.value}</p>
                </div>
              </motion.div>
            ))}
          </div>

          <div className="rounded-[2rem] border border-white/10 bg-white/5 p-8 shadow-lg shadow-black/15 backdrop-blur-xl h-full flex flex-col">
            <h3 className="text-2xl font-semibold text-white">Why should YOU choose me?</h3>
            <ul className="mt-6 space-y-4 text-gray-300 flex-1" role="list">
              {highlights.map((item) => (
                <li key={item} className="flex gap-3" role="listitem">
                  <span className="mt-1 inline-flex h-2.5 w-2.5 rounded-full bg-emerald" aria-hidden="true" />
                  <span>{item}</span>
                </li>
              ))}
            </ul>
      ...
      <div className="mx-auto mt-12 h-0.5 w-3/4 rounded-full bg-sakura-pink/70" />
    </section>
  )
}
